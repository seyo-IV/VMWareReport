##Modules

if(Get-Module -ListAvailable -Name ImportExcel){
    #
    }
else{

$Title = "Info"
$Info = "Install ImportExcel?"
 
$Options = [System.Management.Automation.Host.ChoiceDescription[]] @("&Yes", "&No")
[int]$DefaultChoice = 0
$Opt =  $host.UI.PromptForChoice($Title , $Info, $Options, $DefaultChoice)

switch($Opt)
{
	0 { 
		$check = $true
        Install-Module -Name ImportExcel -Scope CurrentUser -Confirm:$false
	}
	
	1 { 
		$check = $true
        exit 
	}
}
if($check -ne $true){exit}

try{Import-Module ImportExcel}catch{System.Windows.Forms.MessageBox]::Show('ImportExcel Could not be imported.', 'Error', 'Ok', 'Error'); exit}

    }
if(Get-Module -ListAvailable -Name ImportExcel){
    
    }else{exit}
## Functions
Function Select-File {
    Add-Type -AssemblyName System.Windows.Forms
    $SafeFileDialog=New-Object System.Windows.Forms.SaveFileDialog
    $SafeFileDialog.filter = "Excel Files | *.xlsx" 
    $SafeFileDialog.Title = "Choose a save location."
    if($SafeFileDialog.ShowDialog() -eq 'Ok'){

        [System.Windows.Forms.MessageBox]::Show('File is selected.', 'Info', 'Ok', 'Information')
        return $SafeFileDialog.FileName
    }
}

Function Get-VMFolder {
    if($global:defaultviserver -eq $null){
        [System.Windows.Forms.MessageBox]::Show('Connect to VIServer first.', 'Error', 'Ok', 'Error')
    }
    else{
        $Folder = Get-Folder | Sort-Object | Get-Unique
        $Folder_CB.Items.Clear()
        $Folder_CB.Items.Add("All")
        foreach($F in $Folder){
            $Folder_CB.Items.Add($F.Name)
        }
    }
}

Function Get-VMReport {
if($File[1] -ne $null -and $global:defaultviserver -ne $null){
$report = @()
if($Folder_CB.Text -eq "All"){
    $VMs = Get-VM
}
else {
    $VMs = Get-Folder $Folder_CB.Text | Get-VM
}

foreach($vm in $VMs.Name)
    {

    $Progress_LB.text                  = "VM $i of $($VMs.count)"
    $VMData = Get-VM $vm
    $VMIP = $VMData.guest.IPAddress[0]
    $VMIP = [IPAddress]$VMIP
    $data = New-Object PSCustomObject -Property @{
        Name = $VMData.Name;
        State = $VMData.PowerState
        OS = $VMData.ExtensionData.Guest.GuestFullName;
        IPv4 = $VMIP;
        Cores = $VMData.NumCpu;
        RAM = ("$([math]::Round($VMData.MemoryMB)) MB");
        ProvisionedSpace =  ("$([math]::Round($VMData.ProvisionedSpaceGB)) GB");
        UsedSpace =  ("$([math]::Round($VMData.UsedSpaceGB)) GB");
        }
    $report += $data
    $i++
    }
    $xlfile = [string]$File[1]
    $report | select-Object Name, State, OS, IPv4, Cores, RAM, ProvisionedSpace, UsedSpace | Export-Excel $xlfile -AutoSize -StartRow 1 -TableName VMWare
    if($i -eq $($VMs.count)){
        $Progress_LB.text                  = "Export complete."   
    }
}
elseif($File[1] -eq $null -and $global:defaultviserver -ne $null){
    [System.Windows.Forms.MessageBox]::Show('CHoose file first!', 'Error', 'Ok', 'Error')
}
elseif($File[1] -ne $null -and $global:defaultviserver -eq $null){
    [System.Windows.Forms.MessageBox]::Show('Connect to VIServer first!', 'Error', 'Ok', 'Error')
}
}
Function Connect-VMVIServer {
Add-Type -AssemblyName System.Windows.Forms
[System.Windows.Forms.Application]::EnableVisualStyles()

$VIForm                            = New-Object system.Windows.Forms.Form
$VIForm.ClientSize                 = '400,97'
$VIForm.text                       = "Connect to VIServer"
$VIForm.TopMost                    = $false
$VIForm.FormBorderStyle = "FixedDialog"
$VIForm.MaximizeBox = $false
$VIForm.MinimizeBox = $false
$VIForm.StartPosition = "CenterScreen"

$VIServer_TB                     = New-Object system.Windows.Forms.TextBox
$VIServer_TB.multiline           = $false
$VIServer_TB.width               = 100
$VIServer_TB.height              = 20
$VIServer_TB.location            = New-Object System.Drawing.Point(30,44)
$VIServer_TB.Font                = 'Microsoft Sans Serif,10'
$VIServer_TB.Add_KeyDown({
    If($_.KeyCode -eq 'Enter')  {
        $_.SuppressKeyPress = $true
        $VIServer_BT.PerformClick()
    }
})

$VIServer_BT                     = New-Object system.Windows.Forms.Button
$VIServer_BT.text                = "Verbidnen"
$VIServer_BT.width               = 80
$VIServer_BT.height              = 30
$VIServer_BT.location            = New-Object System.Drawing.Point(234,34)
$VIServer_BT.Font                = 'Microsoft Sans Serif,10'
$VIServer_BT.Add_Click{ 
    if($VIServer_TB -ne ""){
        Connect-VIServer $VIServer_TB.Text
        if($global:defaultviserver -ne $null){
			if((Get-View $DefaultViserver.ExtensionData.Client.ServiceContent.SessionManager).count -eq 1){
            $Status_LB.ForeColor 			  = "#008000"
            $Status_LB.text                  = "VIServer connected"
            Get-VMFolder
			}
			else
			{
			$Status_LB.ForeColor 			  = "#008000"
            $Status_LB.text                  = "VIServers connected"
            Get-VMFolder
			}
        }
        else{
            $Status_LB.ForeColor 			  = "#FF0000"
            $Status_LB.text                  = "Not Connected"
        }
        $VIForm.Close()
    }
 }

$VIServer_LB                     = New-Object system.Windows.Forms.Label
$VIServer_LB.text                = "VIServer Name"
$VIServer_LB.AutoSize            = $true
$VIServer_LB.width               = 25
$VIServer_LB.height              = 10
$VIServer_LB.location            = New-Object System.Drawing.Point(36,22)
$VIServer_LB.Font                = 'Microsoft Sans Serif,10'

$VIForm.controls.AddRange(@($VIServer_TB,$VIServer_BT,$VIServer_LB))
$VIForm.ShowDialog()
}

## Form
Add-Type -AssemblyName System.Windows.Forms
[System.Windows.Forms.Application]::EnableVisualStyles()

$Form                            = New-Object system.Windows.Forms.Form
$Form.ClientSize                 = '400,354'
$Form.text                       = "VMWare Report"
$Form.TopMost                    = $false
$Form.FormBorderStyle = "FixedDialog"
$Form.MaximizeBox = $false
$Form.MinimizeBox = $false
$Form.StartPosition = "CenterScreen"

$iconBase64      = ''
$iconBytes       = [Convert]::FromBase64String($iconBase64)
$stream          = New-Object IO.MemoryStream($iconBytes, 0, $iconBytes.Length)
$stream.Write($iconBytes, 0, $iconBytes.Length);
$iconImage       = [System.Drawing.Image]::FromStream($stream, $true)
$Form.Icon       = [System.Drawing.Icon]::FromHandle((New-Object System.Drawing.Bitmap -Argument $stream).GetHIcon())

$Status_LB                       = New-Object system.Windows.Forms.Label
$Status_LB.text                  = "Wait..."
$Status_LB.AutoSize              = $true
$Status_LB.width                 = 25
$Status_LB.height                = 10
$Status_LB.location              = New-Object System.Drawing.Point(200,50)
$Status_LB.Font                  = 'Microsoft Sans Serif,10'

$VIServerConnect_BT              = New-Object system.Windows.Forms.Button
$VIServerConnect_BT.text         = "Connect"
$VIServerConnect_BT.width        = 80
$VIServerConnect_BT.height       = 30
$VIServerConnect_BT.location     = New-Object System.Drawing.Point(66,43)
$VIServerConnect_BT.Font         = 'Microsoft Sans Serif,10'
$VIServerConnect_BT.Add_Click({ Connect-VMVIServer })

$Folder_CB                       = New-Object system.Windows.Forms.ComboBox
$Folder_CB.text                  = ""
$Folder_CB.width                 = 100
$Folder_CB.height                = 20
$Folder_CB.location              = New-Object System.Drawing.Point(44,155)
$Folder_CB.Font                  = 'Microsoft Sans Serif,10'
$Folder_CB.DropDownStyle = "DropDownList"
#$Folder_CB.SelectedIndex = 0

$Connection_GB                   = New-Object system.Windows.Forms.Groupbox
$Connection_GB.height            = 91
$Connection_GB.width             = 367
$Connection_GB.text              = "Connection"
$Connection_GB.location          = New-Object System.Drawing.Point(15,17)

$FIle_BT                         = New-Object system.Windows.Forms.Button
$FIle_BT.text                    = "File"
$FIle_BT.width                   = 60
$FIle_BT.height                  = 30
$FIle_BT.location                = New-Object System.Drawing.Point(244,150)
$FIle_BT.Font                    = 'Microsoft Sans Serif,10'
$FIle_BT.Add_Click({ $global:File = Select-File })

$Progress_LB                     = New-Object system.Windows.Forms.Label
$Progress_LB.text                = "Wait..."
$Progress_LB.AutoSize            = $true
$Progress_LB.width               = 25
$Progress_LB.height              = 10
$Progress_LB.location            = New-Object System.Drawing.Point(76,275)
$Progress_LB.Font                = 'Microsoft Sans Serif,10'

$GO_BT                          = New-Object system.Windows.Forms.Button
$GO_BT.text                     = "Go"
$GO_BT.width                    = 60
$GO_BT.height                   = 30
$GO_BT.location                 = New-Object System.Drawing.Point(244,267)
$GO_BT.Font                     = 'Microsoft Sans Serif,10'
$GO_BT.Add_Click({ Get-VMReport })

$Folder_GB                       = New-Object system.Windows.Forms.Groupbox
$Folder_GB.height                = 73
$Folder_GB.width                 = 160
$Folder_GB.text                  = "Folder"
$Folder_GB.location              = New-Object System.Drawing.Point(15,127)

$File_GB                         = New-Object system.Windows.Forms.Groupbox
$File_GB.height                  = 73
$File_GB.width                   = 187
$File_GB.text                    = "File"
$File_GB.location                = New-Object System.Drawing.Point(194,127)

$Groupbox1                       = New-Object system.Windows.Forms.Groupbox
$Groupbox1.height                = 111
$Groupbox1.width                 = 365
$Groupbox1.text                  = "Export"
$Groupbox1.location              = New-Object System.Drawing.Point(15,224)

$Form.controls.AddRange(@($Status_LB,$VIServerConnect_BT,$Folder_CB,$Connection_GB,$FIle_BT,$Progress_LB,$GO_BT,$Folder_GB,$File_GB,$Groupbox1))


$Form.ShowDialog()
